...
import com.borderless.wallet.socket.chain.bucket_object;
import com.borderless.wallet.socket.chain.object_id;
import com.borderless.wallet.socket.chain.operation_history_object;
...
public class BitshareData {
    ...
    public asset convert_asset_to_base(asset assetAmount, asset_object assetObjectBase) {
        ...
        List<bucket_object> bucketObject = mapAssetId2Bucket.get(assetAmount.asset_id);

        // // TODO: 06/09/2017 这里需要用整数计算提高精度

        long lBaseAmount = (long) (assetAmount.amount * ((double) bucketObject.get(0).close_base / bucketObject.get(0).close_quote));
    ...
    public double get_base_exchange_rate(asset_object assetObject, asset_object assetObjectBase) {
       List<bucket_object> bucketObject = mapAssetId2Bucket.get(assetObject.id);

        double fExchange =
                (double) bucketObject.get(0).close_quote /
                        bucketObject.get(0).close_base *
