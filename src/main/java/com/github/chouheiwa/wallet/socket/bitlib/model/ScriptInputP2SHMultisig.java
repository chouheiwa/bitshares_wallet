...
public class ScriptInputP2SHMultisig extends ScriptInput {
   ...
   protected ScriptInputP2SHMultisig(byte[][] chunks, byte[] scriptBytes) throws ScriptParsingException {
      ...
      byte[][] scriptChunks = chunksFromScriptBytes(_embeddedScript);
      _scriptHash = HashUtils.addressHash(chunks[chunks.length - 1]);
      //the number of signatures needed
      m = opToIntValue(scriptChunks[0]);
      //the total number of possible signing keys
      n = opToIntValue(scriptChunks[scriptChunks.length - 2]);
   ...
   protected static boolean isScriptInputP2SHMultisig(byte[][] chunks) throws ScriptParsingException {
      ...
      byte[][] scriptChunks;
      try {
         scriptChunks = chunksFromScriptBytes(chunks[chunks.length - 1]);
      ...
      if (!isOP(scriptChunks[scriptChunks.length - 1], OP_CHECKMULTISIG)) {
         return false;
      }
      ...
      try {
         int m = opToIntValue(scriptChunks[0]);
         int n = opToIntValue(scriptChunks[scriptChunks.length - 2]);
